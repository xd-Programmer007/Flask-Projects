...
#  I skipped message flashing but skipped it
...
def home():
    ...
    page_per_view = int(params['no_of_posts'])
    last = ceil(len(posts)/page_per_view)
    page = request.args.get('page')
    if(not str(page).isnumeric()):
        page=1
    page = int(page)
    val = (page-1)*page_per_view
    posts = posts[val:(val+page_per_view)]
    if(page==1):
        prev="#"
